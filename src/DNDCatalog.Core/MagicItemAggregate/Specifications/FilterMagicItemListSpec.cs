...
public class FilterMagicItemListSpec : Specification<MagicItem>
{
        ...
        bool? charged)
    {
        ...
        if (priceMin is not null && priceMin > 0) Query.Where(s => s.Price!=null && s.Price.MaxPrice!=null && s.Price!.MaxPrice >= priceMin );
        if (priceMax is not null && priceMax > 0 && priceMax > (priceMin ?? 0)) Query.Where(s => s.Price!=null && s.Price.MinPrice!=null && s.Price!.MinPrice <= priceMax);
